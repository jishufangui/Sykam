...
namespace CKFinder.Connector.CommandHandlers
{
	internal class FileUploadCommandHandler : CommandHandlerBase
	{
		...
		public override void SendResponse( System.Web.HttpResponse response )
		{
			...
			try
			{
				...
				if ( oFile != null )
				{
					...
					if ( Connector.CheckFileName( sFileName ) && !Config.Current.CheckIsHiddenFile( sFileName ) )
					{
                        ...
                        //自定义按时间设置上传目录
                        Regex re = new Regex("[\\w/\\\\]+\\d{2}-\\d{2}-\\d{2}[/\\\\]?", RegexOptions.IgnoreCase);
                        if (!re.IsMatch(sServerDir))
                        {
                            sServerDir = this.CurrentFolder.ServerPath + "/" + String.Format("{0:yyyy-MM-dd}", DateTime.Now) + "/";
                            if (!System.IO.Directory.Exists(sServerDir))
                            {
                                System.IO.Directory.CreateDirectory(sServerDir);
                            }
                        }
                        //自定义结束
			...
			response.Clear();
#endif
            //自定义代码，按时间设置上传目录的另一部分
            string upUrl = this.CurrentFolder.Url;
            Regex re1 = new Regex("[\\w/\\\\]+\\d{2}-\\d{2}-\\d{2}[/\\\\]?", RegexOptions.IgnoreCase);
            if (!re1.IsMatch(upUrl))
            {
                upUrl += String.Format("{0:yyyy-MM-dd}", DateTime.Now) + "/";
            }
            //自定义代码结束
            ...
            //自定义代码部分，下面替换上面被注释掉的文件
            response.Write(this.GetJavaScriptCode(iErrorNumber, sFileName, upUrl + sFileName));
